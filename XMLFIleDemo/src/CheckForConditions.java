		...
		try {
			...
			for (int it1 = 0; it1 < row.getLastCellNum(); it1++) {
				...
				for (int j = 0; j < tagParameterList.getLength(); j++) {
					...
					Node parameter2 = tagParameterList.item(j);
					// getting all the child nodes
					// Node
					NodeList nl = parameter1.getChildNodes();
					NodeList nl2 = parameter2.getChildNodes();
					...
					for (int i = 0; i < nl.getLength(); i++) {
							...
							if (cell.getStringCellValue().equals(node.getTextContent())) {
								...
								for (it2 = 0; it2 < row1.getLastCellNum(); it2++) {
									...
									if (it1 == it2) {
										...
										// {
										// System.out.println("In the nvalue node "+node1.getNodeValue() + "cell2" +
						...
						// for (int nodeNameListVariable = 0;
						// nodeNameListVariable<nodeNameList.getLength(); nodeNameListVariable++)
						// {
